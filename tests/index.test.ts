...
const signer = new ethers.Wallet(process.env.PRIVATE_KEY).connect(ethersProvider)
...
async function getRateLegacy(
  ...
  const pricesURL = `${API_URL}/prices/?from=${srcToken}&to=${destToken}&amount=${amount}${query ? '&' + query : ''}`
...
describe('ParaSwap SDK', () => {
  ...
  test('Get_Rates', async () => {
    const ratesOrError = await paraSwap.getRate('ETH', 'DAI', srcAmount, SwapSide.SELL, { includeDEXS: 'UniswapV2' })
  ...
  test('Get_Allowances', async () => {
    ...
    await Promise.all(allowances.map(allowance => expect(new BigNumber(allowance.allowance).isNaN()).toBe(false)))
  })
  ...
  test('Build_Tx', async () => {
    ...
    const destAmount = new BigNumber(priceRoute.destAmount).times(0.99).toFixed()
  ...
  if (TESTING_ENV) {
    test('Build_tx_legacy', async () => {
      const ratesOrError = await paraSwap.getRate(srcToken, destToken, srcAmount, SwapSide.SELL, {
    ...
    test('Build_and_Send_Tx_BUY', async () => {
      ...
      const ratesOrError = await paraSwap.getRate(srcToken, destToken, destAmount, SwapSide.BUY, {
        includeDEXS: 'Uniswap,UniswapV2,Balancer,Oasis',
      })
      const priceRoute = ratesOrError as OptimalRatesWithPartnerFees
      const _srcAmount = new BigNumber(priceRoute.srcAmount).times(0.99).toFixed(0)
